...
1. **[MÁXIMA PRIORIDAD, EN CURSO] Corregir respuestas erróneas del Test de Teoría parte general (Temas 1-11).** Los alumnos reportan notas mal calculadas; el profesor revisó el Tema 9 a mano y encontró ~12 errores (la revisión real encontró 56). Los PDF oficiales están en `/Users/adrian/Desktop/OPO/OPO 2/Test/GM/2. Parte general Test Vanesa año 1 tema 1 al 11/`.

   **Confirmado un bug sistemático de desplazamiento de índice (+1) en la carga de estos datos**: cuando la respuesta correcta real era la opción a), b) o c), quedó marcada la opción siguiente; cuando era la d) (última), quedó bien por no haber "siguiente" opción. Verificado exhaustivamente, pregunta por pregunta, en **9 de los 11 temas** (Temas 1, 2, 3, 4, 6, 7, 9, 10, 11 — entre 72% y 96% de preguntas erróneas por tema). Las **337 correcciones ya están aplicadas y verificadas en Mongo local** (`opositores_dev`) mediante `scripts/fix_general_theme_answers.py` (commit `a769fd5`), que compara el valor actual antes de tocarlo (0 conflictos) y guarda cada cambio en `edit_history` para auditoría.

   Pendiente:
   - **Temas 5 y 8, y el documento consolidado `Test Adrián 1-11.pdf`**: sus revisiones fallaron por límite de sesión de Claude ("session limit"); relanzar cuando el límite se reset (avisó "resets 8pm Europe/Madrid"). El proceso es: exportar cada tema a JSON (ver `scripts/data/general_theme_answer_fixes_2026_07.json` para el formato de salida esperado) y lanzar un agente por tema comparándolo contra su PDF, igual que se hizo para los otros 9 — luego añadir sus correcciones a ese mismo JSON de datos y volver a correr el script (es idempotente y seguro re-ejecutarlo).
   - **Ejecutar el script contra producción**: el usuario debe correr, con su `.env` de producción (Claude no debe tener esas credenciales):
     ```
     cd backend && source venv/bin/activate && python ../scripts/fix_general_theme_answers.py --dry-run   # comprobar primero
     cd backend && source venv/bin/activate && python ../scripts/fix_general_theme_answers.py              # aplicar de verdad
     ```
