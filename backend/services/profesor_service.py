...
class ProfesorService:
    ...
    async def list_my_students(self, profesor_id: str) -> list:
        students = await self.user_repo.list_by_assigned_profesor(profesor_id)
        result = []
        for s in students:
            all_attempts = await self.exam_repo.get_user_attempts(s["id"])
            attempts = [a for a in all_attempts if a.get("finished_at")]
    ...
    async def get_student_progress(self, student_id: str, profesor_id: str, is_admin: bool = False) -> dict:
        ...
        attempts = await self.exam_repo.get_user_attempts(student_id)
