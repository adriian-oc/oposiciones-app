...
# Un alumno tiene DOS conversaciones independientes: con su profesor asignado, y con
# administración -- antes eran un único hilo compartido (cualquier admin veía y respondía la
# conversación del profesor), pero se separaron a petición del usuario porque la cuenta de
# profesor y la de admin de una misma persona real (ligadas por "cambiar de cuenta") no deben
# mezclar sus mensajes. El "canal admin" se codifica como sufijo ':admin' sobre el mismo
# student_id -- sigue siendo solo una clave de hilo (ver MessageInDB.student_id), Mongo no le da
# ningún significado especial, así que no hace falta migrar los mensajes ya existentes: quedan
# tal cual en el canal 'profesor' (la clave sin sufijo), que es justo lo que se pidió.
ADMIN_CHANNEL_SUFFIX = ":admin"


def _split_thread_key(thread_key: str) -> tuple:
    if thread_key.endswith(ADMIN_CHANNEL_SUFFIX):
        return thread_key[: -len(ADMIN_CHANNEL_SUFFIX)], "admin"
    return thread_key, "profesor"


def _admin_channel_key(user_id: str) -> str:
    return f"{user_id}{ADMIN_CHANNEL_SUFFIX}"
...
class MessageService:
    ...
    async def _authorize(self, current_user: dict, thread_key: str) -> None:
        """Dueño del hilo (ve/escribe siempre el suyo, en cualquiera de sus dos canales). Un
        profesor solo el canal 'profesor' de sus alumnos asignados -- nunca el canal 'admin' de
        nadie, ni ajeno ni de sus propios alumnos, están separados a propósito. Un admin ve el
        canal 'admin' de cualquier alumno y el hilo propio de cualquier profesor -- nunca el
        canal 'profesor' de un alumno, eso es privado entre el alumno y su profesor."""
        owner_id, channel = _split_thread_key(thread_key)
        ...
        if role == "admin":
            ...
            if owner and owner.get("role") == "profesor":
                return  # hilo propio de un profesor con administración
        elif role == "profesor" and channel == "profesor":
            owner = await self.user_repo.get_by_id(owner_id)
    ...
    async def _relevant_thread_ids(self, current_user: dict) -> list:
        """Hilos que le tocan a este usuario -- alumno los dos canales suyos (profesor y admin),
        profesor el suyo propio (con admin) más el canal 'profesor' de sus alumnos asignados,
        admin el canal 'admin' de todos los alumnos y el hilo propio de todos los profesores."""
        role = current_user["role"]
        if role == "student":
            return [current_user["id"], _admin_channel_key(current_user["id"])]
        ...
        if role == "admin":
            return (
                [_admin_channel_key(u["id"]) for u in users if u.get("role") == "student"]
                + [u["id"] for u in users if u.get("role") == "profesor"]
            )
        return []
    ...
    def _counterpart(self, thread_key: str, current_user: dict, users_by_id: dict) -> dict:
        """'Con quién se habla' en un hilo, para el header del chat y la barra de contactos --
        distinto de 'quién soy yo dentro del hilo': un alumno mirando su propio hilo ve a su
        profesor (canal 'profesor') o a 'Administración' (canal 'admin'), no a sí mismo; un
        profesor mirando el suyo ve 'Administración' (cualquier admin puede responder, no hay
        uno fijo, ver _authorize)."""
        owner_id, channel = _split_thread_key(thread_key)
        if owner_id == current_user["id"]:
    ...
    async def _send_digest_after_delay(self, thread_key: str, window_start: datetime) -> None:
        try:
            await asyncio.sleep(DIGEST_WINDOW_SECONDS)
            owner_id, _channel = _split_thread_key(thread_key)
            owner = await self.user_repo.get_by_id(owner_id)
            if not owner or not owner.get("email"):
                return
            thread = await self.message_repo.get_thread(thread_key)
            new_messages = [
                m for m in thread
                if m["created_at"] >= window_start and m["sender_id"] != owner_id
            ]
            if not new_messages:
                return
            ...
            # Enlace directo al canal exacto (perfil/profesor o admin) -- /profesor/chat/:id
            # ahora acepta también role=student (ver App.js), así el alumno puede entrar
            # directo a cualquiera de sus dos conversaciones desde el correo.
            chat_link = f"{settings.frontend_base_url}/profesor/chat/{thread_key}"
            message_payload = [
                {
